...
export async function removeProjectFileFromGptRagAndOpenAi(supabase, projectId, row, opts) {
  ...
  if (!fid || !openaiApiKey) return { openai_detached: false };
  ...
  if (!vsId) return { openai_detached: false };
  ...
  try {
    ...
    return { openai_detached: true };
  } catch (e) {
    return { openai_detached: false, error: e.response?.data?.error?.message || e.message };
  }
}
