...
export function buildProjectFileCatalogAppendix(rows) {
  ...
  return (
    '\n\n[System — project document index (names in this project only). Map the user\'s question to these files when they ask about a specific document. User-visible answers must be verbatim quotes from file_search snippets (no paraphrase of document wording).]\n' +
    unique.map((n) => `· ${n}`).join('\n') +
    '\n'
  );
}
