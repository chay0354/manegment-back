...
/**
 * Safety ceiling for one project sync. Default raised so "ask on all project files" does not
 * silently exclude older files; override via MANEGER_GPT_RAG_MAX_FILES when needed.
 */
export const GPT_RAG_MAX_FILES = (() => {
  const raw = parseInt(String(process.env.MANEGER_GPT_RAG_MAX_FILES || ''), 10);
...
export async function syncProjectGptRagToOpenAI(supabase, projectId, opts) {
  ...
  const eligibleCandidates = (files || []).filter(isProjectFileGptRagEligible);
  if (eligibleCandidates.length > GPT_RAG_MAX_FILES) {
    return {
      ok: false,
      status: 400,
      error:
        `Project has ${eligibleCandidates.length} GPT-searchable files, above current sync cap (${GPT_RAG_MAX_FILES}). ` +
        'Raise MANEGER_GPT_RAG_MAX_FILES on the server, then sync again.'
    };
  }
