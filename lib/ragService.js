...
export class RAGService {
  ...
  async generateAnswer(query, nResults = 10, filterMetadata = null, useLlm = true) {
    const results = await this.search(query, nResults, filterMetadata);
      ...
      try {
        const res = await axios.post(
          'https://api.openai.com/v1/chat/completions',
          {
            model: 'gpt-4o-mini',
            messages: [
              { role: 'system', content: 'Answer the question based only on the context. Respond in Hebrew (עברית).' },
