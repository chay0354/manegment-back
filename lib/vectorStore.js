      ...
      if (filterMetadata) {
        ...
        for (const [k, v] of Object.entries(filterMetadata)) {
          if (k === 'filenames' && Array.isArray(v) && v.length > 0) {
            ...
            // Delete/re-ingest: exact logical name only. LIKE on basename removed every chunk sharing the same
            // basename (e.g. two different paths ending in report.pdf).
            conds.push(`metadata->>'filename' = $${i}`);
            params.push(v);
            i++;
          } else if (v != null) {
